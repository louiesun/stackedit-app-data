...
## 翻墙
### 根本原理：流量转发
为了避开GFW的审查，最简单的思路是流量转发,就好像如果A不能直接写给B,
就写哈☰C、内容是
以plaintext
请转发给B
原内容
#井井困难一:流量识别.
显然,反动政府会想尽一切办法阻止我们绕过审查。同样地,其不☰敢破坏正常
流量的通行,因此,其气☰尝试寻找翻墙流量的特征。
以上面A发给C的信为例,每次都说:“请转发给…”特征十分明显,
特征,并不局限于内容,甚至每个包的大小也是特征。
有的朋友可能说:“既然个人了可以加密内容何不加密信件内容呢?”但要注意,往往☰的
内容也是TLS加密的。这叫“TLSinTL5”。
这里介绍一点,TLS握手包的大小是明显的,而加密基本不改变包大小,因此,“TLsinTL”
的前几个包大小☰规律明显,已被GFW识别。
为了防止流是识别,就需要设计协议。这里的协议就同之屋的切议一样,规定了交流。
一般翻磅协议建立在TCP上,初兴规定了客户端与代理服务器如何交流
...
Telegraom。Gongle手机注册疑似要求手机号与2P同国家,Telgram如果一个帐号ZP多次
改变(想第一个人一会儿在美国,一会儿去了印度尼西亚。☰),可能导致封号;一些国内
服务限制外国人使用。同志们接码时需注意。
当然,自由平台不大会做这类限制。
井井常见翻墙软件介绍。(具体界面操作此处略去)
 Vr2ray, ☰Singbox.
这两个是主流的机场客户端。(往往其代现服务器也是这个软件,此外不深入)
所谓节点,可以认为是一份这类软件的配置,告诉软件如何接机场的代理服
所谓机场,就是用一些服务器(通常是及的)搭建了一匹节点用于出售。
这类软件是开源的,但机场主人不排除是晶哥。当然,此时泄漏的信息同直连相同。
...
非教家Tor并无必要,且多次转发,解容速度极慢,原本网页加载可延长至30s.
WebRTC泄露IP问题
...
出口墙
在北上,个的国际出口,被认为有6FW出口墙。其负责IP黑☰洞,封锁一部分域名。
☰☰省墙
一般认为各省甚至☰地级市的
封锁互相独交。
省墙进行DNS污染,☰封锁更备城多,封锁一部分2P。
由于少教民族地区民族矛盾,一般此处封锁更严重。
井GFW的连接方式
GFW的电脑一般认为主要是并联。但ID封锁之类(非黑洞)似乎只能用串联解释。
井GFW☰策略
很有意思,被识别的☰有的时候不被立即封锁,而是秋后☰喝茶。
☰☰☰
一般修敏感时期,如两会,六四,国庆封锁变严。但对未识别协议无影响。
似乎一个印流是大了(如共同节点)会被重点关注。
封锁发生在天,大概,动参
...
Proton UPN,Geph
这是两个推存的与中修,无肉身关系(指作者非国人)的自由VPN,官方提供免费服务,
速度适中,上XMPP,看网页(非视频)不成问题。
同时,这两项JPV也去打隐私,甚至采用了一些技术手段一定程度上保证这一点。
需要注意,☰protonWNv☰时最好确保选择stealth协议,
Winegwand
协议特征☰明显。
默认
###小牛VP1,老天VPN等
不开源,极不安全,如老王PN作者极有可能为网警
无专业人员
指导切记不要使用☰☰☰☰,其可能☰携带病毒。
尽管做好证书校验,泄雾信息与直连相同。但不排除其直接从俯手机获取信息
上传。对一些使用自由系统的同志,这使你刷机大概功亏一号。
而且你一般也不至于用这个。此类中N可☰断连,但上面的不咋会。
Lantem等
...
井网络问题快速诊断
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjQ0NzYxMjI0LC0xODg4MzUxOTEwLC02Mz
I5NTIyNTNdfQ==
-->